...
function db_connect() 
{
  ...
  $idcnx = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME/**/);
//var_dump($cnx);
  mysqli_select_db($idcnx,DB_NAME ) or trigger_error(mysqli_connect_error());
  if($idcnx->connect_errno){
    echo $idcnx->connect_error;
}
  
/*  $idcnx=new PDO('mysql:host='.DB_HOST.';dbname='.DB_NAME,DB_USER,DB_PASSWORD,array(PDO::MYSQL_ATTR_INIT_COMMAND=>'SET NAMES "utf8"'));
...
function db_query($sql) 
{
  ...
  $res = mysqli_query($cnx, $sql) or trigger_error( mysqli_connect_error() )/**/;
